...
<div class="space-y-8" x-data="formBuilder">
        ...
        <div class="relative flex items-center justify-between">
            <div class="flex items-center gap-6">
                <div class="w-16 h-16 bg-gradient-to-r from-orange-500 to-red-600 rounded-2xl flex items-center justify-center shadow-lg">
                    <x-heroicon-s-pencil class="h-8 w-8 text-white"/>
                </div>
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ $companyType->name }} Bewerken</h1>
                    <p class="text-gray-600 dark:text-slate-400 mt-1">Wijzig dit bedrijfstype</p>
                </div>
            </div>
            <span class="inline-flex items-center rounded-xl bg-blue-500/20 px-4 py-2 text-sm font-bold text-blue-700 dark:text-blue-300">
                ID: {{ $companyType->id }}
            </span>
        </div>
    </div>
    ...
    <form action="{{ route('portal.admin.companies.types.update', $companyType) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="w-full bg-gray-100/50 dark:bg-slate-800/50 border border-gray-200 dark:border-slate-600 rounded-xl px-4 py-3 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">{{ old('description', $companyType->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
