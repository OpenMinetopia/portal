...
@section('title', 'Bedrijfstype Bewerken')
@section('header', 'Bedrijfstype Bewerken')
...
@section('content')
    <div class="space-y-6" x-data="formBuilder">
        ...
        <form action="{{ route('portal.admin.companies.types.update', $companyType) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg">
                        ...
                        <div class="px-4 py-5 sm:p-6 space-y-6">
                            ...
                            <div>
                                ...
                                <div class="mt-1">
                                    <textarea name="description" id="description" rows="3"
                                              class="appearance-none block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-white">{{ old('description', $companyType->description) }}</textarea>
                                </div>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    ...
                                    <input type="number" name="price" id="price"
                                           value="{{ old('price', $companyType->price) }}"
                                           min="0" step="0.01"
                            ...
                            <!-- Status Toggle -->
                            <div x-data="{ enabled: {{ json_encode(old('is_active', $companyType->is_active)) }} }">
                                <div class="flex items-center justify-between">
                ...
                <div class="space-y-6">
                                    ...
                                    <div class="relative flex items-start">
                                        <div class="flex h-6 items-center">
                                            <input type="checkbox" name="authorized_roles[]" id="role_{{ $role->id }}"
                                                   value="{{ $role->id }}"
                                                   @checked(in_array($role->id, old('authorized_roles', $companyType->authorized_roles ?? [])))
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg">
                            <div class="px-4 py-5 sm:p-6">
                                <button type="button"
                                        onclick="if(confirm('Weet je zeker dat je dit bedrijfstype wilt verwijderen?')) { document.getElementById('delete-form').submit(); }"
        ...
        <form id="delete-form" action="{{ route('portal.admin.companies.types.destroy', $companyType) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('formBuilder', () => ({
                    fields: @json(old('form_fields', $companyType->form_fields ?? [])),
                    addField() {
                        this.fields.push({
                            type: 'text',
                            label: '',
