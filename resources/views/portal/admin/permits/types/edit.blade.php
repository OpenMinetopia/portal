...
@section('title', 'Vergunning Type Bewerken')
@section('header', 'Vergunning Type Bewerken')
...
@section('content')
    <div class="space-y-6" x-data="formBuilder">
        ...
        <form action="{{ route('portal.admin.permits.types.update', $permitType) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                            ...
                            <div>
                                ...
                                <div class="mt-1">
                                    <textarea name="description" id="description" rows="3"
                                              class="appearance-none block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm dark:bg-gray-700 dark:text-white">{{ old('description', $permitType->description) }}</textarea>
                                </div>
                                @error('description')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <div class="mt-1 relative rounded-md shadow-sm">
                                    ...
                                    <input type="number" name="price" id="price"
                                           value="{{ old('price', $permitType->price) }}"
                                           min="0" step="0.01"
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex-grow flex flex-col">
                                        ...
                                        <span class="text-sm text-gray-500 dark:text-gray-400">
                                            Deze vergunning is beschikbaar voor aanvragen
                                        </span>
                                    </div>
                                    <button type="button"
                            ...
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                        Voeg velden toe die ingevuld moeten worden bij het aanvragen
                                    </p>
                                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg">
                        <div class="px-4 py-5 sm:px-6 border-b border-gray-200 dark:border-gray-700">
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                Selecteer welke rollen deze vergunningen mogen beheren
                            </p>
                        </div>
                        <div class="px-4 py-5 sm:p-6">
                            <div class="space-y-4">
                                    ...
                                    <div class="relative flex items-start">
                                        <div class="flex h-6 items-center">
                                            <input type="checkbox" name="authorized_roles[]" id="role_{{ $role->id }}"
                                                   value="{{ $role->id }}"
                                                   @checked(in_array($role->id, old('authorized_roles', $permitType->authorized_roles ?? [])))
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg">
                            <div class="px-4 py-5 sm:p-6">
                                <form action="{{ route('portal.admin.permits.types.update', $permitType) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <!-- Hidden fields to preserve form data -->
                                    <input type="hidden" name="name" value="{{ old('name', $permitType->name) }}">
                                    <input type="hidden" name="description" value="{{ old('description', $permitType->description) }}">
                                    <input type="hidden" name="price" value="{{ old('price', $permitType->price) }}">
                                    <input type="hidden" name="is_active" :value="enabled">
                                    <template x-for="(field, index) in fields" :key="index">
                                        <div>
                                            <input type="hidden" :name="`form_fields[${index}][label]`" :value="field.label">
                                            <input type="hidden" :name="`form_fields[${index}][type]`" :value="field.type">
                                            <input type="hidden" :name="`form_fields[${index}][required]`" :value="field.required ? 1 : 0">
                                            <template x-if="field.type === 'select'">
                                                <template x-for="(option, optionIndex) in field.options" :key="optionIndex">
                                                    <input type="hidden" :name="`form_fields[${index}][options][]`" :value="option">
                                                </template>
                                            </template>
                                        </div>
                                    </template>
                                    @foreach($roles as $role)
                                        @if(in_array($role->id, old('authorized_roles', $permitType->authorized_roles ?? [])))
                                            <input type="hidden" name="authorized_roles[]" value="{{ $role->id }}">
                                        @endif
                                    @endforeach
                        ...
                        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg">
                            <div class="px-4 py-5 sm:p-6">
                                <form action="{{ route('portal.admin.permits.types.destroy', $permitType) }}" 
                                      method="POST" 
                                      onsubmit="return confirm('Weet je zeker dat je dit vergunning type wilt verwijderen?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                            class="w-full inline-flex justify-center rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 dark:bg-red-500 dark:hover:bg-red-400">
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('formBuilder', () => ({
                    fields: @json(old('form_fields', $permitType->form_fields ?? [])),
                    addField() {
                        this.fields.push({
                            type: 'text',
                            label: '',
