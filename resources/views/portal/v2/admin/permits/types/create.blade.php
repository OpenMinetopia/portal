...
@section('title', 'Nieuw vergunningtype toevoegen')
...
<div class="space-y-8">
    ...
    <div class="border-b border-white/10 pb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-white">Nieuw vergunningtype</h1>
                <p class="text-slate-400 mt-2">Voeg een nieuw type vergunning toe aan het systeem</p>
            </div>
    ...
    <!-- Form -->
    <div class="max-w-4xl">
        <form action="{{ route('portal.admin.permits.types.store') }}" method="POST" class="space-y-8">
            @csrf
            ...
            <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-8">
                ...
                <div class="mt-6">
                    ...
                    <textarea id="description" 
                              ...
                              rows="4"
                              class="w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-3 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                              placeholder="Beschrijf waarvoor deze vergunning gebruikt wordt...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="requirements" 
                                  ...
                                  rows="4"
                                  class="w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-3 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                                  placeholder="Lijst de vereisten op die nodig zijn voor deze vergunning...">{{ old('requirements') }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea id="required_documents" 
                                  ...
                                  rows="3"
                                  class="w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-3 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                                  placeholder="Lijst de documenten op die meegestuurd moeten worden...">{{ old('required_documents') }}</textarea>
