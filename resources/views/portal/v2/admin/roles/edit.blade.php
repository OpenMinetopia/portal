...
@section('title', 'Rol bewerken - ' . $role->name)
...
<div class="space-y-8">
    ...
    <div class="border-b border-white/10 pb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-white">Rol bewerken</h1>
                <p class="text-slate-400 mt-2">Bewerk de instellingen van "{{ $role->display_name ?? $role->name }}"</p>
            </div>
            <div class="flex items-center space-x-4">
                <a href="{{ route('portal.admin.roles.show', $role) }}" 
    ...
    <!-- Form -->
    <div class="max-w-4xl">
        <form action="{{ route('portal.admin.roles.update', $role) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-8">
                ...
                <div class="mt-6">
                    ...
                    <textarea id="description" 
                              ...
                              rows="3"
                              class="w-full bg-slate-800/50 border border-slate-600 rounded-lg px-4 py-3 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                              placeholder="Beschrijf wat deze rol inhoudt...">{{ old('description', $role->description) }}</textarea>
                    @error('description')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
